...
export class SimpleGraph {
  ...
  endCanvas = undefined;
  public contentFillStyle = "#FFFFFF";
  public contentTitle = "TITLE";
  public contentStrokeStyle = "#222222";
  public chartFillStyle = "#FFFFFF";
  public chartStrokeStyle = "#000000";
  ...
  public chartAxisXFnc = (data: any) => {
    return data;
  }; //화면에 값뿌려줄때 어떻게 뿌려줄꺼냐
  public chartAxisYFnc = (data: any) => {
    return data;
  };
  public chartDataVisible = true; //차트에 값표시할꺼냐
  public chartAxisXDataMin = undefined; //셋팅되지않으면 데이터값의 min max값으로처리
  public chartAxisXDataMax = undefined;
  public chartAxisYDataMin = undefined;
  public chartAxisYDataMax = undefined;
  public chartAxisXDataMinMarginPercent = 10;
  public chartAxisXDataMaxMarginPercent = 10;
  public chartAxisYDataMinMarginPercent = 10;
  public chartAxisYDataMaxMarginPercent = 10;
  ...
  public constructor(targetCanvas: HTMLCanvasElement | string) {
    if (typeof targetCanvas === 'string') {
      this.canvas = document.getElementById(targetCanvas) as HTMLCanvasElement;
    } else {
...
//				useThis.context.fillText("    "+atEndData.y+",  "+atEndData.x, dragEndPoint.x, dragEndPoint.y);
  ...
  rendering() {
    this.endCanvas = Utils.copyCanvas(this.canvas);
...
//	this.context.fillStyle = "#aa0000";  this.context.fill();
  ...
  drawChartLineData(graphKData) {//GraphDataK
...
//	this.context.closePath();
  ...
  drawChartBezoerLineData(graphKData) {//GraphDataK
...
//	this.context.closePath();
  ...
  drawChartLineFillData(graphKData) {//GraphDataK
  ...
  drawChartLineGroupData(graphKData) {//GraphDataK
    this.context.strokeStyle = graphKData.strokeStyle;
    this.context.fillStyle = graphKData.strokeStyle; //스타일있으면 그걸로셋팅.
    this.context.beginPath();
...
//	this.context.closePath();
  ...
  drawChartDotData(graphKData) {//GraphDataK
    this.context.strokeStyle = graphKData.strokeStyle;
    this.context.fillStyle = graphKData.strokeStyle; //스타일있으면 그걸로셋팅.
    this.context.textAlign = "center";
    this.context.textBaseline = "middle";
  ...
  drawChartArcData(graphKData) {//GraphDataK
    this.context.strokeStyle = graphKData.strokeStyle;
    this.context.fillStyle = graphKData.strokeStyle; //스타일있으면 그걸로셋팅.
    this.context.textAlign = "center";
    this.context.textBaseline = "middle";
  ...
  drawChartStickData(graphKData) { //GraphDataK
    this.context.strokeStyle = graphKData.strokeStyle;
    this.context.fillStyle = graphKData.strokeStyle; //스타일있으면 그걸로셋팅.
    this.context.beginPath();
...
//차트데이터 데이터값을주면  상대좌표를 픽셀을 돌려준다.  복합.
  getDrawChartPoints(graphKData) {
    ...
    for (var i = 0; i < dataArray.length; i++) {
      ...
      var point = this.getDrawChartPoint(
        new Point(atData[graphKData.xVarName], atData[graphKData.yVarName]),
  ...
  drawChartCustomData(graphKData) {//GraphDataK
    this.context.strokeStyle = graphKData.strokeStyle;
    this.context.fillStyle = graphKData.strokeStyle; //스타일있으면 그걸로셋팅.
    this.context.textAlign = "center";
    this.context.textBaseline = "middle";
...
//차트데이터 데이터값을주면  상대좌표를 픽셀을 돌려준다.
  getDrawChartPoint(point, xMin, xMax, yMin, yMax) {
    ...
    yMin = Number(yMin);
    yMax = Number(yMax);
    var yData_BetweenLength = Utils.getBetweenLength(yMin, yMax);
    var xData_BetweenLength = Utils.getBetweenLength(xMin, xMax);

    var yMinMargin = Utils.getValueByTotInPercent(yData_BetweenLength, this.chartAxisYDataMinMarginPercent);
    var yMaxMargin = Utils.getValueByTotInPercent(yData_BetweenLength, this.chartAxisYDataMaxMarginPercent);
    var xMinMargin = Utils.getValueByTotInPercent(xData_BetweenLength, this.chartAxisXDataMinMarginPercent);
    var xMaxMargin = Utils.getValueByTotInPercent(xData_BetweenLength, this.chartAxisXDataMaxMarginPercent);
    ...
    yData_BetweenLength = Utils.getBetweenLength(yMin, yMax);//yMinMargin;
    xData_BetweenLength = Utils.getBetweenLength(xMin, xMax);//xMinMargin;
    ////////

    var yAtData_BetweenLength = Utils.getBetweenLength(yMin, point.y); //ex) 10(원하는거) - 7(민) = 3(차)
    var xAtData_BetweenLength = Utils.getBetweenLength(xMin, point.x);

    var yDataPercent = Utils.getPercentByTot(yData_BetweenLength, yAtData_BetweenLength); // 전체차에서  원하는값의차는 몇%인가
    var xDataPercent = Utils.getPercentByTot(xData_BetweenLength, xAtData_BetweenLength);
    var yPoint = Utils.getValueByTotInPercent(this.chartDataRect.height, yDataPercent);//전체값의 몇 퍼센트는 얼마? 계산법 공식 ;  전체값 X 퍼센트 ÷ 100
    var xPoint = Utils.getValueByTotInPercent(this.chartDataRect.width, xDataPercent);
    var ySet = this.chartDataRect.getEndY() - yPoint;
    var xSet = this.chartDataRect.getStartX() + xPoint;

    if (ySet && xSet) {
      return new Point(xSet.toFixed(2), ySet.toFixed(2));
    } else {
...
//차트데이터 안쪽에서의 상대좌표를 픽셀을주면  이에따른   데이터 값을 돌려준다.
  getDrawChartData(point, xMin, xMax, yMin, yMax) {
    xMin = Number(xMin);
    xMax = Number(xMax);
    yMin = Number(yMin);
    yMax = Number(yMax);
    ...
    var yPointPercent = Utils.getPercentByTot(this.chartDataRect.height, point.y); // 전체차에서  원하는값의차는 몇%인가
    var xPointPercent = Utils.getPercentByTot(this.chartDataRect.width, point.x);

    var yData = Utils.getValueByTotInPercent(yData_BetweenLength, yPointPercent);//전체값의 몇 퍼센트는 얼마? 계산법 공식 ;  전체값 X 퍼센트 ÷ 100
    var xData = Utils.getValueByTotInPercent(xData_BetweenLength, xPointPercent);
...
//	this.context.textBaseline	= "end";
    ...
    if (this.chartAxisScaleVisible) {
      ...
      var xBetweenLength = Utils.getBetweenLength(this.chartDataRect.getStartX(), this.chartDataRect.getEndX());
      var yBetweenLength = Utils.getBetweenLength(this.chartDataRect.getStartY(), this.chartDataRect.getEndY());
      ...
      var xDataBetweenLength = Utils.getBetweenLength(this.data.getDataXMin(), this.data.getDataXMax());
      var yDataBetweenLength = Utils.getBetweenLength(this.data.getDataYMin(), this.data.getDataYMax());
      ...
      var xMinBetweenLength = Utils.getBetweenLength(this.data.getDataXMin(), xMin);
      var yMinBetweenLength = Utils.getBetweenLength(this.data.getDataYMin(), yMin);
      ...
      var xMaxBetweenLength = Utils.getBetweenLength(this.data.getDataXMin(), xMax);
      var yMaxBetweenLength = Utils.getBetweenLength(this.data.getDataYMin(), yMax);
      ...
      var xMinPercent = Utils.getPercentByTot(xDataBetweenLength, xMinBetweenLength);
      var yMinPercent = Utils.getPercentByTot(yDataBetweenLength, yMinBetweenLength);
      ...
      var xMaxPercent = Utils.getPercentByTot(xDataBetweenLength, xMaxBetweenLength);
      var yMaxPercent = Utils.getPercentByTot(yDataBetweenLength, yMaxBetweenLength);
      ...
      var startX = Utils.getValueByTotInPercent(xBetweenLength, xMinPercent);
      var startY = Utils.getValueByTotInPercent(yBetweenLength, yMinPercent);
      ...
      var setWidth = Utils.getValueByTotInPercent(this.chartDataRect.width, xMaxPercent);
      var setHeight = Utils.getValueByTotInPercent(this.chartDataRect.height, yMaxPercent);
